    ...
    <div class="auth-right py-32 px-24 d-flex flex-column justify-content-center">
        <div class="max-w-464-px mx-auto w-100">
            <div>
                ...
                <h4 class="mb-12">{{ Translation::getTranslation('Sign In to your Account') }}</h4>
                <p class="mb-32 text-secondary-light text-lg">{{ Translation::getTranslation('Welcome back! please enter your detail') }}</p>
            </div>
                ...
                <div class="d-flex justify-content-between gap-2">
                    <div class="form-check style-check d-flex align-items-center">
                        <input class="form-check-input border border-neutral-300" type="checkbox" name="remember"
                            value="1" id="remeber">
                        <label class="form-check-label" for="remeber">{{ Translation::getTranslation('Remember me') }} </label>
                    </div>
                    <a href="{{ route('password.request') }}" class="text-primary-600 fw-medium">{{ Translation::getTranslation('Forgot
                        Password?') }}</a>
                </div>

                <button type="submit" class="btn btn-primary text-sm btn-sm px-12 py-16 w-100 radius-12 mt-32">
                    {{ Translation::getTranslation('Sign In') }}</button>
